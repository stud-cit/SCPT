              ...
              <v-card>
                <v-card-text>
                  <v-container grid-list-md>
                    <v-layout wrap>
                      <v-flex xs12 sm6 md4>
                        <v-text-field v-model="NewCourse.title" label="Name"></v-text-field>
                      </v-flex>
                      <v-flex xs12 sm6 md4>
                        <v-text-field v-model="NewCourse.data" label="Data"></v-text-field>
                      </v-flex>
                    </v-layout>
                  </v-container>
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="blue darken-1" text @click="CloseCourse">Cancel</v-btn>
      ...
      <v-flex xs12>
        <v-card>
          <v-toolbar flat color="white">
            ...
            <v-spacer/>
            <v-dialog v-if="isEmpty(selected)" v-model="DialogValidate" max-width="500px">
              <template v-slot:activator="{ on }">
                <v-btn color="primary" dark class="mb-2" v-on="on">
                  <v-icon>mdi-plus</v-icon>
                  New Item
              ...
              <v-card>
                  ...
                  <v-container grid-list-md>
                    <v-layout wrap>
                      <v-flex xs12>
                        ...
                        <v-text-field v-model="NewUser.course" label="Course"></v-text-field>
                        <v-text-field v-model="NewUser.email" label="Email"></v-text-field>
                        <v-text-field v-model="NewUser.phone" label="Phone"></v-text-field>
                        <v-text-field v-model="NewUser.data" label="Data"></v-text-field>
                      </v-flex>
                    </v-layout>
                  </v-container>
                </v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="blue darken-1" text @click="CloseValidate">Cancel</v-btn>
                  <v-btn color="blue darken-1" text @click="SaveValidate">Save</v-btn>
                </v-card-actions>
              </v-card>
            </v-dialog>
          </v-toolbar>
          ...
          <template v-slot:items="item">
            ...
            <td class="text-xs-right">
              <v-icon
                small
                @click="DeleteValidate(item.item)"
                >
                mdi-delete
              </v-icon>
            </td>
      ...
      <v-flex xs12>
        <v-card>
          <v-toolbar flat color="white">
            ...
            <v-spacer/>
            <v-dialog v-if="isEmpty(selected)" v-model="DialogNotValidate" max-width="500px">
              <template v-slot:activator="{ on }">
                <v-btn color="primary" dark class="mb-2" v-on="on">
                  <v-icon>mdi-plus</v-icon>
                  New Item
              ...
              <v-card>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="blue darken-1" text @click="CloseNotValidate">Cancel</v-btn>
                  <v-btn color="blue darken-1" text @click="SaveNotValidate">Save</v-btn>
                </v-card-actions>
              </v-card>
            </v-dialog>
          </v-toolbar>
          ...
          <template v-slot:items="item">
            ...
            <td class="text-xs-right">
              <v-icon
                small
                @click="DeleteNotValidate(item.item)"
                >
                mdi-delete
              </v-icon>
            </td>
...
export default class AdminPage extends Vue {
    ...
    courses = new Array(5).fill('').map((item,i)=> ({
      ...
      DeleteValidate (item) {
        ...
        confirm('Are you sure you want to delete this item?') && this.selected.validate.splice(index, 1)
      };
      DeleteNotValidate (item) {
        const index = this.selected.notvalidate.indexOf(item)
        confirm('Are you sure you want to delete this item?') && this.selected.notvalidate.splice(index, 1)
      };
